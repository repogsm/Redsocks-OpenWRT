...
return L.view.extend({
	...
	load: function() {
		return Promise.all([
			L.resolveDefault(fs.exec('/etc/init.d/redsocks', ['enabled']), { code: 1 }),
			L.resolveDefault(fs.exec('/etc/init.d/redsocks', ['running']), { code: 1 })
		]);
	},

	handleServiceAction: function(action, ev) {
		return fs.exec('/etc/init.d/redsocks', [action]).then(function(res) {
			if (res.code !== 0)
